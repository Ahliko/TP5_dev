...
def dynamic_header(msg: int) -> bytes:
    return binaire(len(binaire(msg))) + binaire(msg)


def send(lst, signe):
    header_int = len(binaire(lst[0])).to_bytes(2, byteorder='big')
    header = dynamic_header(len(header_int + binaire(lst[0]) + signe.encode() + binaire(lst[1])))
    print(len(header))
    print(header)
